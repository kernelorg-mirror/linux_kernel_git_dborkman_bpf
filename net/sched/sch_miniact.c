...
static void miniact_put(struct bpf_prog *prog)
{
	if (prog)
		bpf_prog_put(prog);
}
			    ...
			    struct netlink_ext_ack *extack)
{
	...
	if (!ret) {
		mini_qdisc_pair_swap(&mq->miniqp, NULL, req->prog);
		if (!before && swap_offload)
...
static int miniact_dump_prog(struct sk_buff *skb,
			     const struct miniact_miniq *miniact,
			     const struct mini_Qdisc *miniq)
{
	u32 flags = 0;

	if (nla_put_u32(skb, TCA_MINIACT_PARMS_PROGS, miniq->entry->aux->id))
		return -1;
...
static int miniact_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	if (miniq && miniact_dump_prog(skb, &q->ingress, miniq))
		goto nla_put_failure;
	...
	if (miniq && miniact_dump_prog(skb, &q->egress, miniq))
		goto nla_put_failure;
	nla_nest_end(skb, nest_inner);
...
static const struct nla_policy bpf_parms_policy[TCA_MINIACT_PARMS_MAX + 1] = {
	[TCA_MINIACT_PARMS_PROGS]	= { .type = NLA_U32 },
...
static int miniact_get_prog(struct net_device *dev,
			    struct miniact_swap_req *req,
			    struct nlattr *prog)
{
	int prog_fd = nla_get_u32(prog);
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	if (tb[TCA_MINIACT_PARMS_FLAGS]) {
		u32 flags = nla_get_u32(tb[TCA_MINIACT_PARMS_FLAGS]);
	...
	return miniact_get_prog(dev, req, tb[TCA_MINIACT_PARMS_PROGS]);
}
